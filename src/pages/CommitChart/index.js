...
import * as req from '../../utils/request/commit'
...
export default class CommitChart extends Component {
  ...
  componentDidMount() {
    // req.repos('life1st').then(({data}) => {
    //   console.log(data)
    //   this.setState({
    //     repos: data
    //   })
    //   data.map(repo => {
    //     req.commits('life1st', repo.name, 'life1st', new Date('2018-12-07')).then(({data}) => {
    //       console.log(data)
    //       this.setState({
    //         commits: {
    //           ...this.state.commits,
    ...
    GET('https://api.github.com/repos/life1st/markIt/commits').then(({data}) => {
      this.setState({
        commits: {
          'fake': data
        }
